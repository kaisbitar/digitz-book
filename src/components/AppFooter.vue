<template>
  <v-footer height="40" app>
    ...
    <div v-if="dataReady">
      <!-- Data is ready! First Surah: {{ firstSurahName }} -->
    </div>
  <button @click="testPersistence">Test Persistence</button>
...
<script setup>
  ...
  // Compute a value from the store data
  const firstSurahName = computed(() => {
    if (quranStore.getTableQuranIndex.length > 0) {
      return quranStore
    }
    return ''
  })
  ...
  quranStore.target.fileName = 'TestFile'
  console.log('Store updated:', quranStore)
}
  onMounted(async () => {
    const result = await prepareData()
    console.log(result)
    console.log('First Surah:', firstSurahName)
  })
</script>
